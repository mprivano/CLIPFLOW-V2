...
async function handleVizardPublishRequest(request: any, apiKey: string) {
  ...
  if (!publishSuccessful) {
    ...
    if (!isVizardReachableVideoUrl(videoUrl)) {
      throw new Error("Vizard fallback could not access this private/local clip URL. Use Direct TikTok publish for Google Drive clips.");
    }
...
function isVizardReachableVideoUrl(videoUrl: string) {
  try {
    ...
    if (host.includes("googleusercontent.com") || host.includes("googleapis.com")) return false;
    if (parsed.pathname.includes("/api/proxy-video")) return false;
    return true;
  } catch {
    return false;
  }
